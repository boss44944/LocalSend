...
make_mac() {
  arch=$1
  label=$2
  app='work/LAN Clipboard.app'
  rm -rf "$app"
  mkdir -p "$app/Contents/MacOS" "$app/Contents/Resources"

  GOOS=darwin GOARCH="$arch" CGO_ENABLED=0 go build -trimpath -ldflags='-s -w' -o "$app/Contents/MacOS/clipboard-bin" .

  cat > "$app/Contents/MacOS/clipboard" <<'EOF'
#!/bin/sh
DATA_DIR="$HOME/Library/Application Support/LAN Clipboard"
mkdir -p "$DATA_DIR"
exec "$(dirname "$0")/clipboard-bin" -data-dir "$DATA_DIR" "$@"
EOF
...
EOF
  ...
  chmod +x "$app/Contents/MacOS/clipboard" "$app/Contents/MacOS/clipboard-bin"
